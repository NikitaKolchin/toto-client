...
export default class AuthService {
    static async login({ email, password }: AuthDto): Promise<AxiosResponse<AuthResponse>> {
        return $api.post<AuthResponse>('/auth/login', { email, password })
    }

    static async registration({ email, password, name }: RegDto): Promise<AxiosResponse<AuthResponse>> {
